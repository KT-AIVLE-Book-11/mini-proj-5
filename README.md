...
| `GET` | `/books/{bookId}/comments` | 댓글 목록 조회 | 특정 ID의 도서에 등록된 모든 댓글 데이터 최신순 조회 |
| `POST` | `/books/{bookId}/comments` | 댓글 작성 | 특정 ID의 도서에 새 댓글 등록(생성일 자동 초기화) |
...
| **DETAIL** | **도서 상세 페이지** | 도서 상세 정보 조회, 내용/표지 개별 수정, 도서 삭제 및 조회수/좋아요 실시간 반영, 댓글 조회/등록 |
